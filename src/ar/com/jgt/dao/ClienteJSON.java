...
public class ClienteJSON implements GrabableJSON<ClienteDTO>{
	...
	private List<JSONObject> m_jsonList = null;
	...
	public String createJSON(ClienteDTO p_json) {
		...
		m_jsonList = new LinkedList<JSONObject>(); 									//Creamos una lista para almacenar JSON
		m_jsonList.addAll(Arrays.asList(m_jsonObj));									//Se añade a la lista los objetos JSON
		m_jsonString = JSONValue.toJSONString(m_jsonList); 							//Generamos el String JSON		
